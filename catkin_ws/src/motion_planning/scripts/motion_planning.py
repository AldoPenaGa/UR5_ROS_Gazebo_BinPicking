...
def get_gazebo_model_name(model_name, vision_model_pose):
    ...
    raise ValueError(f"Modelo {model_name} en posicion {vision_model_pose.position.x} {vision_model_pose.position.y} no fue encontrado!")
...
def straighten(model_pose, gazebo_model_name):
    ...
    print(f"Lego está frente a {facing_direction}")
...
if __name__ == "__main__":
    print("Inciando nodo cinemático")
    ...
    print("Moviendo a posición READY")
    ...
    print("Esperando para detectar los módelos")
    rospy.sleep(0.5)
    ...
    for model_name, model_pose in legos:
        ...
        try:
            ...
            print(f"Nombre del módelo {model_name} no fue reconocido!")
            continue
        ...
        print(f"Moviendo el módelo {model_name} a {x} {y} {z}")
